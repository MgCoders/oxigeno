...
function capital_logo() {
    $info = capital_get_logo_information();
    ...
    if ( get_theme_mod( 'logo-retina-ready', capital_get_default( 'logo-retina-ready' ) ) ) {
        $width /= 2;
        $height /= 2;
    }
 ...
 */
function capital_has_logo() {
    $information = capital_get_logo_information();
 ...
 */
function capital_get_logo_information( $force = false ) {
    $logo_information = array();

    $logo_information['image'] = get_theme_mod( 'logo', capital_get_default( 'logo' ) );

    if ( ! empty( $logo_information['image'] ) ) {
        $dimensions = capital_get_logo_dimensions( $logo_information['image'], $force );
...
function capital_get_logo_dimensions( $url, $force = false ) {
    // Build the cache key
    $key = 'capital-' . md5( 'logo-dimensions-' . $url . WPZOOM::$themeVersion );
    ...
    if ( false === $dimensions || is_preview() || true === $force ) {
        ...
        $attachment_id = capital_get_attachment_id_from_url( $url );
 ...
 */
function capital_get_attachment_id_from_url( $url = '' ) {
 ...
 * @return void
 */
function capital_display_favicons() {
    $logo_favicon = get_theme_mod( 'logo-favicon', capital_get_default( 'logo-favicon' ) );
    if ( ! empty( $logo_favicon ) ) : ?>
        <link rel="icon" href="<?php echo esc_url( $logo_favicon ); ?>"/>
    <?php endif;
}

add_action( 'wp_head', 'capital_display_favicons' );
